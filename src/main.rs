...
fn main() -> Result<()> {
    ...
    let mut video  = VIDEO {info: String::new(), title: String::new(), mime: String::new()};
    ...
    for handler in inventory::iter::<&dyn definitions::SiteDefinition> {
        ...
        if !video_exists {
            ...
            if vt.is_empty() {
                ...
                let url = handler.find_video_direct_url(&mut video, in_url, webdriverport, args.onlyaudio)?;
                let ext =
                    handler.find_video_file_extension(&mut video, in_url, webdriverport, args.onlyaudio)?;
